...
public class BirdsOfAFeatherNode extends SearchNode {
	...
	public BirdsOfAFeatherNode() {
		this(new Random().nextInt());
	}
	
	public BirdsOfAFeatherNode(int seed) {
		this(seed, DEFAULT_SIZE);
	}
	
	public BirdsOfAFeatherNode(int seed, int size) {
		this(seed, size, size);
	}
	
	public BirdsOfAFeatherNode(int seed, int numRows, int numColumns) {
...
//		System.out.println(this);
	 ...
	 */
	public BirdsOfAFeatherNode makeMove(int row1, int col1, int row2, int col2) {
		BirdsOfAFeatherNode child = (BirdsOfAFeatherNode) childClone(); 
     ...
     */
	public Object clone() {
		BirdsOfAFeatherNode newNode = (BirdsOfAFeatherNode) super.clone();
...
//				sb.append(grid[r][c] == null ? "       " : String.format("%s(%2d) ", grid[r][c].toString(), stackSize[r][c]));
	...
	public void buildSolutionString(StringBuilder sb) {
		if (parent != null) {
			((BirdsOfAFeatherNode) parent).buildSolutionString(sb);
			sb.append(" ");
			sb.append(prevMove);
		}	
	}
	...
	public boolean checkOddBird(int row, int col) {
		Card card = grid[row][col];
		for(int i = 0; i<grid.length; i++) {
			for(int j=0; j<grid[i].length; j++) {
				if(!((i == row) && (j == col))) {
					Card oddOne = grid[i][j];
					if(card.getSuit() == oddOne.getSuit()) {
						return false;
					} else {
						if((card.getRank() == oddOne.getRank()) || (card.getRank() == (oddOne.getRank()-1)) || (card.getRank() == (oddOne.getRank()+1))) {
							return false;
						}
					}
				}
    ...
    public static void main(String[] args){
    	...
    	SearchNode root = new BirdsOfAFeatherNode(367297990);
    	...
    	if (searcher.search(root)) {
    		...
    		System.out.println("Solution string: " + ((BirdsOfAFeatherNode) searcher.goalNode).solutionString());
    	} else {
    		// unsuccessful search
